SET(ALGORITHM_PROFILE_UID_Namespace "de.dkfz.matchpoint.common")
SET(ALGORITHM_PROFILE_UID_Name "Dummy.2D.Image")
SET(ALGORITHM_PROFILE_UID_Version "1.0.0")
#SET(ALGORITHM_PROFILE_UID_Tag "") #Is optional

SET(ALGORITHM_PROFILE_Description "Simple registration algorithm that generate a 2D unity transform (Registration changes nothing).")
SET(ALGORITHM_PROFILE_Contact "Ralf Floca\; sbr@dkfz-heidelberg.de")
#SET(ALGORITHM_PROFILE_Terms "")
#SET(ALGORITHM_PROFILE_Citation "")

SET(ALGORITHM_PROFILE_DataType "Image")
#SET(ALGORITHM_PROFILE_ComputationStyle "")
SET(ALGORITHM_PROFILE_Deterministic "1")
SET(ALGORITHM_PROFILE_ResolutionStyle "Single")
SET(ALGORITHM_PROFILE_DimMoving "2")
SET(ALGORITHM_PROFILE_ModalityMoving "any")
SET(ALGORITHM_PROFILE_DimTarget "2")
SET(ALGORITHM_PROFILE_ModalityTarget "any")
SET(ALGORITHM_PROFILE_Subject "any")
SET(ALGORITHM_PROFILE_Object "any")
SET(ALGORITHM_PROFILE_TransformModel "rigid")
SET(ALGORITHM_PROFILE_TransformDomain "global")
#SET(ALGORITHM_PROFILE_Optimization "none")
#SET(ALGORITHM_PROFILE_Interaction "none")
SET(ALGORITHM_PROFILE_Keywords "basic" "test")
