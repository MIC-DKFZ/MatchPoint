...
SET(ALGORITHM_PROFILE_Description "This is a MatchPoint wrapper for the registration tool Plastimatch for 3D images. It is configured to preform a B-Spline registration on the image. REMARK: You need the plastimatch command line tool installed on this computer to run this algorithm. You can specify the plastimatch location 1) via the algorithm property PlastimatchDirectory or 2) via setting the environment variable MAP_PLASTIMATCH_PATH.")
