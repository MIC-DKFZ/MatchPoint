...
{
	std::cout << "MatchPoint Elastix testing dummy." << std::endl <<
			  "This is a mock up exe, used by MatchPoint to test its Elastix integration." << std::endl <<
			  std::endl;
	...
	//write dummy result
	file.open(map::core::FileDispatch::createFullPath(outputPath, "TransformParameters.0.txt").c_str(),
			  iOpenFlag);
