...
// Subversion HeadURL: $HeadURL: http://sidt-hpc1/dkfz_repository/NotMeVisLab/SIDT/MatchPoint/trunk/Code/Algorithms/ITK/include/mapITKFRPROptimizer.h $
*/




#ifndef __ITK_FRPR_OPTIMIZER_H
#define __ITK_FRPR_OPTIMIZER_H
...
namespace map
{
	namespace algorithm
	{
		namespace itk
		{
			...
			/*!
			This class is the template specialization for the itk::FRPROptimizer.
			@ingroup ITK
			@ingroup OptimizerControl
			*/
			template<>
			class ITKOptimizerControl< ::itk::FRPROptimizer> : public ITKSVNLOptimizerControlBase< ::itk::FRPROptimizer>
			{
			public:
				typedef ITKOptimizerControl< ::itk::FRPROptimizer> Self;
				typedef ITKSVNLOptimizerControlBase< ::itk::FRPROptimizer> Superclass;
			...
			protected:
				...
				virtual void copyProperties(const ConcreteOptimizerType *source, ConcreteOptimizerType *destination) const
				{
					...
					mapLogInfoMacro( << "Cloning assumes that the FRPR optimizer should be cloned with set to Fletch Reeves style (default). Because there is no possibility to deduce the style of the source instance.");
				};
			...
			typedef ITKOptimizerControl< ::itk::FRPROptimizer> FRPROptimizerControl;
