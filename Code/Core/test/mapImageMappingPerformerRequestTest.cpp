...
namespace map
{
	namespace testing
	{
		int mapImageMappingPerformerRequestTest(int, char* [])
		{
			...
			typedef core::ImageMappingPerformerRequest<RegistrationType, InputImageType, InputImageType>
			RequestType;
			...
			RequestType::ResultImageDescriptorType::Pointer spDescriptor =
				RequestType::ResultImageDescriptorType::New();

			typedef itk::LinearInterpolateImageFunction<InputImageType, core::continuous::ScalarType>
			InterpolatorType;
			InterpolatorType::Pointer spInterpolator = InterpolatorType::New();

			// create a first RequestType object
			RequestType request1(spRegistration, spInputImage, spDescriptor, spInterpolator,
								 _throwOnMappingError, _errorValue, _throwOnOutOfInputAreaError, _paddingValue);
