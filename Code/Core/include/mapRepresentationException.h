...
// Subversion HeadURL: $HeadURL: http://sidt-hpc1/dkfz_repository/NotMeVisLab/SIDT/MatchPoint/trunk/Code/Core/include/mapRepresentationException.h $
*/
	...
	namespace core
	{
		...
		*/
		...
		public:
			...
			/*! Constructor. Needed to ensure the exception object can be copied. */
			RepresentationException(const std::string &file, unsigned int lineNumber) : Superclass(file, lineNumber) {}

			/*! Constructor. Needed to ensure the exception object can be copied. */
			RepresentationException(const std::string &file, unsigned int lineNumber, const std::string &desc, const std::string &loc) : Superclass(file, lineNumber, desc, loc) {}
