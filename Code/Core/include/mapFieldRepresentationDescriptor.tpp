...
namespace map
{
  namespace core
  {
      ...
      streamToSDInternal() const
    {
      ...
      structuredData::Element::Pointer spSpacingElement = structuredData::streamITKFixedArrayToSD(this->getSpacing());
      ...
      streamFromSDInternal(const structuredData::Element* pElement)
    {
      ...
      newSize = SizeType::streamFromSD(*subPos);
			...
			if (subPos == pElement->getSubElementEnd())
			{
				mapDefaultExceptionMacro(<< "Error: cannot stream from structured data. Reason: sub element \"Origin\" is missing.");
			}
      newOrigin = structuredData::streamSDToITKFixedArray<PointType>(*subPos);
			...
			if (subPos == pElement->getSubElementEnd())
			{
				mapDefaultExceptionMacro(<< "Error: cannot stream from structured data. Reason: sub element \"Spacing\" is missing.");
			}
      newSpacing = structuredData::streamSDToITKFixedArray<SpacingType>(*subPos);
			...
			if (subPos == pElement->getSubElementEnd())
			{
				mapDefaultExceptionMacro(<< "Error: cannot stream from structured data. Reason: sub element \"Direction\" is missing.");
			}
