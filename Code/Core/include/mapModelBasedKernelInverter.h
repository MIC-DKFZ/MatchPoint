...
// Subversion HeadURL: $HeadURL: http://sidt-hpc1/dkfz_repository/NotMeVisLab/SIDT/MatchPoint/trunk/Code/Core/include/mapModelBasedKernelInverter.h $
*/

#ifndef __MAP_MODEL_BASED_KERNEL_INVERTER_H
#define __MAP_MODEL_BASED_KERNEL_INVERTER_H
...
namespace map
{
	namespace core
	{
		/*! @class ModelBasedKernelInverter
		* @brief Provider that is able to invert a ModelBasedRegistrationKernel.
		*
		* If the tranformation model of the given kernel can be inverted analyticaly
		* the inverter will create ModelBasedRegistrationKernel based on the model inversion.
		* If the inversion is not possible the inverter will create a LazyFieldBasedRegistrationKernel
		* an populates it with a parametrized FieldByModelInversionFunctor.
		*
		...
		*/
		...
		class ModelBasedKernelInverter : public RegistrationKernelInverterBase<VInputDimensions, VOutputDimensions>
		{
		public:
			/*! Standard class typedefs. */
			typedef ModelBasedKernelInverter<VInputDimensions, VOutputDimensions>				Self;
			typedef RegistrationKernelInverterBase<VInputDimensions, VOutputDimensions>	Superclass;
			typedef itk::SmartPointer<Self>        Pointer;
			typedef itk::SmartPointer<const Self>  ConstPointer;

			itkTypeMacro(ModelBasedKernelInverter, RegistrationKernelInverterBase);
			itkNewMacro(Self);
			...
			typedef ModelBasedRegistrationKernel<VInputDimensions, VOutputDimensions> KernelType;
		...
		protected:
			...
			virtual ~ModelBasedKernelInverter() {};

		private:
			ModelBasedKernelInverter(const Self &); //purposely not implemented
