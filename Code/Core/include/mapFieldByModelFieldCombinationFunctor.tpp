		...
		namespace functors
		{
			...
			generateField() const
			{
				...
				for (iterator.GoToBegin(); !iterator.IsAtEnd(); ++iterator)
				{
					...
					typename SourceModelKernelType::OutputPointType interimPoint = _spSourceModelKernel->transformPoint(
								inPoint);

					typename SourceModelKernelType::OutputVectorType firstVector;
					PointVectorCombinationPolicy<VInputDimensions, VInterimDimensions>::computeVector(inPoint,
							interimPoint, firstVector);
					...
					if (!spInterpolator->IsInsideBuffer(interimPoint))
					{
						if (! CombinationInterface::_usePadding)
						{
							...
							mapExceptionMacro(RepresentationException,
											  << "Cannot combine kernels. Point is not inside supported region of second kernel. Point: " <<
											  interimPoint);
						}
			...
			template <unsigned int VInputDimensions, unsigned int VInterimDimensions, unsigned int VOutputDimensions>
			const typename
			FieldByModelFieldCombinationFunctor<VInputDimensions, VInterimDimensions, VOutputDimensions>::SourceFieldKernelType*
			...
			template <unsigned int VInputDimensions, unsigned int VInterimDimensions, unsigned int VOutputDimensions>
			const typename
			FieldByModelFieldCombinationFunctor<VInputDimensions, VInterimDimensions, VOutputDimensions>::SourceModelKernelType*
			...
			template <unsigned int VInputDimensions, unsigned int VInterimDimensions, unsigned int VOutputDimensions>
			const typename
			FieldByModelFieldCombinationFunctor<VInputDimensions, VInterimDimensions, VOutputDimensions>::SourceKernel1BaseType*
			...
			template <unsigned int VInputDimensions, unsigned int VInterimDimensions, unsigned int VOutputDimensions>
			const typename
			FieldByModelFieldCombinationFunctor<VInputDimensions, VInterimDimensions, VOutputDimensions>::SourceKernel2BaseType*
				...
				const InFieldRepresentationType* pInFieldRepresentation)
			{
				...
				Pointer spFieldByModelFieldCombinationFunctor = new Self(modelKernel, fieldKernel,
			...
			CreateAnother(void) const
			{
				...
				Pointer spNew = Self::New(*_spSourceModelKernel, *_spSourceFieldKernel,
			...
			FieldByModelFieldCombinationFunctor(const SourceModelKernelType& modelKernel,
												const SourceFieldKernelType& fieldKernel,
												const InFieldRepresentationType* pInFieldRepresentation):
				Superclass(pInFieldRepresentation), _spSourceModelKernel(&modelKernel),
				_spSourceFieldKernel(&fieldKernel)
			{
				assert(pInFieldRepresentation);
			}
