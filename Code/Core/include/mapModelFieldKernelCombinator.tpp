...
namespace map
{
	namespace core
	{
					   ...
					   const PaddingVectorType& paddingVector) const
		{
			...
			typedef functors::FieldByModelFieldCombinationFunctor<VInputDimensions, VInterimDimensions, VOutputDimensions>
			FunctorType;
		...
		{
			OStringStream os;
			os << "ModelFieldKernelCombinator<" << VInputDimensions << "," << VInterimDimensions << "," <<
			   VOutputDimensions << ">";
			return os.str();
		}
		...
		{
			OStringStream os;
			os << "ModelFieldKernelCombinator, VInputDimensions: " << VInputDimensions <<
