        ...
        * Thus 2D DICOM images are missed as well as 3D DICOM images that are stored as one image file (default series style).*/
        ...
        static typename PointSetType::Pointer doPointSetMapping(const ApplicationData& appData)
        {
          ...
          try
          {
            ...
            std::cout << std::endl << "map input file... ";
            spMapper->setInputPointSet(pCastedInput);
            spMapper->setRegistration(pCastedReg);
            if (appData._noFailOnErrors)
            {
              spMapper->setThrowOnMappingError(false);
              spMapper->setErrorPointValue(appData._paddingValue);
            }
            spMapper->execute();
            spResult = spMapper->getResultPointSet();
            std::cout << "done." << std::endl;
          }
          catch (::itk::ExceptionObject& e)
          {
        ...
        static void processData(const ApplicationData& appData)
        {
          ...
          {
            typename PointSetType::Pointer mappedPoints = doPointSetMapping(appData);
            std::cout << std::endl << "save output file ... ";
            ::map::utilities::saveLandMarksToFile<PointSetType>(appData._outputFileName, mappedPoints);
